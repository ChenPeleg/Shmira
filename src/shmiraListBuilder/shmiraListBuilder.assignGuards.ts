...
export const ShmiraListBuilderBuildNightsAndUnAssigned = (
  preferencesMeta: PreferenceMetaDataModel[],
  ...
  const nights: NightScheduleModel[] = allDatesArray.map(
    (d: string, i: number) => {
      const optionalGuards = preferencesMeta
        .filter((g) => g.datesYouCanGuard.includes(d))
        .map((g) => g.id);
      ...
      optionalGuards.forEach((guard) => {
        ...
        if (guard.preference.halfOrFull === "2") {
          ...
          ShmiraListBuilderTools.checkIfPersonCanGuard(
            guard,
            n.date,
            buildSettings.daysBetweenGuardDuty
          )
        ) {
          // @ts-ignore
          guard.guardDates.push(n.date);
  ...
  const assignedPreferences = preferencesMeta.filter(
    (p) => p.guardDates.length >= 2
  );
  const unassignedPreferences = preferencesMeta.filter(
    (p) => p.guardDates.length < 2
  );
  return {
