...
describe('ShmiraList rename import Dialog', () => {
    ...
    beforeEach(async () => {
        ...
        store = mockStore({});
        fileDialog = (<Provider store={store}><RenameDialog selectedValue={shmiraListDefaultName} open={true} key={'1'}
                                                            onClose={onClose}/>
        </Provider>);
        render(fileDialog);
    ...
    it('closes dialog on press cancel', async () => {
        const cancelButton =
            await screen.getByTestId('shmiraList-rename-cancel-button') as HTMLElement;
    ...
    it('entering value and pressing approve triggers dispatch',  async () => {
       ...
       await act(async  () => {
            const input : HTMLInputElement = component.querySelector('input#shmiraList-rename-dialog-text-field') as HTMLInputElement;
            input.value = 'rename shmiraList';
            const aprvButton = await component.querySelector('#shmiraList-rename-approve-button') as HTMLElement;
