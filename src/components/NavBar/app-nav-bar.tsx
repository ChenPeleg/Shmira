...
export const AppNavBar = () => {
  ...
  const [ManageShmiraListimOpen, setManageShmiraListimOpen] =
    React.useState(false);
  const [importPreferencesOpen, setImportPreferencesOpen] =
    React.useState(false);
  ...
  const importSheetsDialogOpen =
    useSelector(
      (state: ShmiraListStore) =>
        state.currentSessionState.isImportSheetModalOpen
    ) || false;

  const shmiraListSelected = shmiraListCollection.find(
    (shmiraListRecord: ShmiraListRecord) => shmiraListRecord.id === shmiraListId
  );
  const nextShmiraListId = Utils.getNextId(
    shmiraListCollection.map((c) => c.id)
  );
  ...
  const handleShmiraListMenuClick = (
    event: React.MouseEvent<HTMLElement>,
    clickAction: ShmiraListActionType
  ) => {
    switch (clickAction) {
  ...
  return (
    <Box dir="rtl">
              ...
              <IconButton
                ...
                onClick={() =>
                  handleProfileMenuClose(
                    null,
                    ProfileMenuClickActionType.Import
                  )
                }
                color="inherit"
              >
            ...
            <LightTooltip title={translations.ExportToFile}>
              <IconButton
                ...
                onClick={() =>
                  handleProfileMenuClose(
                    null,
                    ProfileMenuClickActionType.Export
                  )
                }
                color="inherit"
              >
      ...
      <ShmiraListMenu
        shmiraListMoreAnchorEl={shmiraListMoreAnchorEl}
        shmiraListMenuId={shmiraListMenuId}
      ...
      <FileUploadDialog
        ...
        selectedValue={""}
      />
      <ShmiraListManagementDialog
        open={ManageShmiraListimOpen}
        onClose={() => {
          setManageShmiraListimOpen(false);
        }}
      />
      <PreferenceImportDialog
        open={importPreferencesOpen}
        onClose={() => {
