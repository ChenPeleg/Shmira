...
export const Utils = {
  ...
  defaultSketchMMock(): SketchModel {
    ...
    const mkDrv = (
      ...
      finish: string,
      location: string,
      TypeOfInfoPreference: PreferenceType = PreferenceType.CanGuardIn
    ): ExtendedPreferenceModel => {
      ids++;
      return {
        id: ids.toString(),
  ...
  Date: {
    ...
    dateToDateStamp(date: Date): string {
      ...
      let dateStamp =
        Math.floor(
          (resultDate.getTime() - resultDate.getTimezoneOffset() * 60 * 1000) /
            milisecondsInDay
        ) -
        Math.round(
          (startDate.getTime() - startDate.getTimezoneOffset()) /
            milisecondsInDay
        );
    ...
    getTimestampArrayFromStartAndFinishDate(
      dateFrom: string,
      dateTo: string
    ): string[] {
      const numberOfDays: number = Number(dateTo) - Number(dateFrom) + 1;
      const correctedNumberOfDays =
        numberOfDays < 100 && numberOfDays > 10 ? numberOfDays : 40;
    ...
    getWeekDayNumberFromTimeStamps(
      timeStamps: string[]
    ): { timeStamp: string; dayNumber: number }[] {
    ...
    getDatOfWeekTextFromTimeStamp(timeStamps: string): string {
      ...
      return (
        this.dateOfWeekObject.find((d) => d.weekDayNumber === dayNumber)
          ?.name || ""
      );
    },
