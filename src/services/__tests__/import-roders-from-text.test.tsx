...
describe('import-orders-from-text', () => {
    it('creates prefernces properly form sheet input last', () => {
        ...
        expect(!!(foundYair && foundGilad)).eq(true, 'did not found two guards in the list');
        expect(foundYair?.weekDaysOrDates == '2').eq(true, 'guard is not weekDaysOrDates == 2');
        expect(foundGilad?.weekDaysOrDates == '2').eq(true, 'guard is not weekDaysOrDates == 2');
        expect(foundYair?.flexibilityByDates.join(',')).eq(Yair.flexibilityByDates.join(','), 'flexability is not calculated properly');
        expect(foundGilad?.flexibilityByDates.join(',')).eq(Gilad.flexibilityByDates.join(','), 'flexability is not calculated properly');
