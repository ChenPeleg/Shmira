import {AppConstants, defaultPreferenceValues, IAction, ShmiraListStore} from './store.types';
...
export const PreferenceReducer: Record<PreferenceReducerFunctions, (state: ShmiraListStore, action: IAction) => ShmiraListStore> = {
    ...
    [ActionsTypes.CLONE_ORDER]: (state: ShmiraListStore, action: IAction): ShmiraListStore => {
        ...
        const cloneOriginPreference = newState.preferences.find(preference => cloneOriginId === preference.id);
        if (cloneOriginPreference) {
            const preferenceIndex = newState.preferences.indexOf(cloneOriginPreference) + 1;
            const newId = Utils.getNextId(getAllPreferencesIDs(state))
            const newPreference: PreferenceModel = {
    ...
    [ActionsTypes.UPDATE_ORDER]: (state: ShmiraListStore, action: IAction): ShmiraListStore => {
        ...
        newState.preferences = newState.preferences.map(preference => {
            if ((preferenceId === preference.id) && newState.dataHolderForCurrentPreferenceInEdit) {
    ...
    [ActionsTypes.DELETE_ORDER]: (state: ShmiraListStore, action: IAction): ShmiraListStore => {
        ...
        newState.preferences = newState.preferences.filter(preference => deletePreferenceId !== preference.id)
        if (newState.dataHolderForCurrentPreferenceInEdit && newState.dataHolderForCurrentPreferenceInEdit.id === deletePreferenceId) {
            newState.dataHolderForCurrentPreferenceInEdit = null;
        }
