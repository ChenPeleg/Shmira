...
export type ShmiraListReducerFunctions =
  ...
  | ActionsTypes.UPDATE_DAYS_BETWEEN;
...
export const ShmiraListReducer: Record<
  ShmiraListReducerFunctions,
...
> = {
  [ActionsTypes.CHOOSE_SIDUR]: (
    ...
    const chosenShmiraListObj: ShmiraListRecord | undefined =
      newState.shmiraListCollection.find(
        (record: ShmiraListRecord) => record.id === chosenShmiraListId
      );
    if (chosenShmiraListObj !== undefined) {
      const previousShmiraListObj: ShmiraListRecord | undefined =
        newState.shmiraListCollection.find(
          (record: ShmiraListRecord) => record.id === previousShmiraListId
        );
      if (previousShmiraListObj !== undefined) {
        const NewPreviousShmiraListObj = { ...previousShmiraListObj };
        NewPreviousShmiraListObj.preferences = newState.preferences.map(
          (o) => ({
            ...o,
          })
        );
        NewPreviousShmiraListObj.deletedPreferences =
          newState.deletedPreferences.map((o) => ({
            ...o,
          }));
        NewPreviousShmiraListObj.sketches = newState.sketches.map((o) => ({
          ...o,
        }));
        NewPreviousShmiraListObj.daysBetweenGuardDuty =
          newState.daysBetweenGuardDuty;
  ...
  [ActionsTypes.DELETE_SHMIRA]: (
    ...
    if (shmiraListIdToDelete.includes(AppConstants.ArchiveIdPrefix)) {
      newState.shmiraListArchive = newState.shmiraListArchive.map(
        (shmiraList) => {
          if (shmiraListIdToDelete === shmiraList.id) {
            const updatedShmiraList = { ...shmiraList };
            updatedShmiraList.id = updatedShmiraList.id.replace(
      ...
      let deletedShmiraList: ShmiraListRecord | undefined =
        newState.shmiraListCollection.find(
          (s) => s.id === shmiraListIdToDelete
        );
      ...
      newState.shmiraListCollection = newState.shmiraListCollection.filter(
        (s) => s.id !== shmiraListIdToDelete
      );
    }
    ...
    if (newState.shmiraListId === shmiraListIdToDelete) {
      const chosenShmiraListAfterDelete: ShmiraListRecord =
        newState.shmiraListCollection[0];
  ...
  [ActionsTypes.ARCHIVE_SIDUR]: (
    ...
    if (shmiraListIdToArchive.includes(AppConstants.deleteIdPrefix)) {
      ...
      let archivedShmiraList: ShmiraListRecord | undefined =
        newState.shmiraListCollection.find(
          (s) => s.id === shmiraListIdToArchive
        );
      if (archivedShmiraList) {
        archivedShmiraList = { ...archivedShmiraList };
        archivedShmiraList.id =
          AppConstants.ArchiveIdPrefix + archivedShmiraList.id;
    ...
    if (newState.shmiraListId === shmiraListIdToArchive) {
      const chosenShmiraListAfterArchive: ShmiraListRecord =
        newState.shmiraListCollection[0];
  ...
  [ActionsTypes.CLONE_SIDUR]: (
    ...
    let shmiraListForCloning: ShmiraListRecord | undefined =
      newState.shmiraListCollection.find((s) => s.id === shmiraListIdToClone);

    if (shmiraListForCloning) {
      const newShmiraList: ShmiraListRecord =
        CloneUtil.deepCloneShmiraList(shmiraListForCloning);
  ...
  [ActionsTypes.MOVE_TO_ACTIVE_SIDUR]: (
    ...
    let toActivateShmiraList: ShmiraListRecord | undefined =
      newState.shmiraListArchive.find((s) => s.id === shmiraListIdToActivate);
    if (toActivateShmiraList) {
      toActivateShmiraList = { ...toActivateShmiraList };

      toActivateShmiraList.id = StoreUtils.removeIdPrefix(
        toActivateShmiraList.id
      );
      newState.shmiraListCollection.push(toActivateShmiraList);
      newState.shmiraListArchive = newState.shmiraListArchive.filter(
        (s) => s.id !== shmiraListIdToActivate
      );
    }
  ...
  [ActionsTypes.DELETE_FOREVER_SIDUR]: (
    ...
    newState.shmiraListArchive = newState.shmiraListArchive.filter(
      (s) => s.id !== shmiraListIdToDeleteForever
    );
    StoreUtils.HandleReducerSaveToLocalStorage(newState);
    return newState;
  },
};
const setChosenShmiraList = (
  state: ShmiraListStore,
  chosenShmiraList: ShmiraListRecord
): ShmiraListStore => {
  const newState = { ...state };

  newState.preferences =
    chosenShmiraList?.preferences.map((o) => ({ ...o })) || [];
  newState.daysBetweenGuardDuty = chosenShmiraList?.daysBetweenGuardDuty;
  newState.deletedPreferences =
    chosenShmiraList?.deletedPreferences?.map((o) => ({ ...o })) || [];
  newState.sketches = chosenShmiraList?.sketches?.map((o) => ({ ...o })) || [];
  if (newState.SketchIdInEdit) {
    const sketchInEdit = newState.sketches.find(
      (s) => s.id === newState.SketchIdInEdit
    );
    if (sketchInEdit) {
...
const getDefaultShmiraList = (state: ShmiraListStore): ShmiraListRecord => {
  ...
  const allNames = [
    ...state.shmiraListCollection.map((o) => o.Name),
    ...state.shmiraListArchive.map((o) => o.Name),
  ];
